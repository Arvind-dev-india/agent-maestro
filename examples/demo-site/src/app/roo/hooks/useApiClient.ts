...
const logApiResponse = (url: string, status: number, data?: any) => {
  console.log(
    `📥 [${new Date().toISOString()}] API Response: ${status} for ${url}`,
  );
  if (data) {
    const logData =
...
const logApiError = (
  ...
  console.error(
    `❌ [${new Date().toISOString()}] API Error: ${method} ${url}`,
    {
      error: {
...
const makeApiRequest = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 502) {
        ...
        (error as any).userMessage =
          "Connection failed. Please check if the VS Code extension is running.";
    ...
    if (error.name === "TypeError" && error.message.includes("fetch")) {
      ...
      (networkError as any).userMessage =
...
export const useApiClient = () => {
  const sendTaskAction = useCallback(
    async (taskId: string, suggestion: string): Promise<boolean> => {
      ...
      try {
        console.log(
          `🎯 [${new Date().toISOString()}] Sending task action: ${action} for task ${taskId}`,
        );
        ...
        console.log(
          `✅ [${new Date().toISOString()}] Task action sent successfully: ${action} for task ${taskId}`,
        );
        return true;
      } catch (error: any) {
        console.error(
          `💥 [${new Date().toISOString()}] Failed to send task action:`,
          {
            taskId,
            action,
            error: error.message,
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        console.log(
          `💬 [${new Date().toISOString()}] Sending message${taskId ? ` to task ${taskId}` : " (new task)"}`,
        );
        ...
        console.log(
          `✅ [${new Date().toISOString()}] Message sent successfully${taskId ? ` to task ${taskId}` : " (task created)"}`,
        );

        // Add connection monitoring for SSE responses
        if (response.headers.get("content-type") === "text/event-stream") {
          console.log(
            `📡 [${new Date().toISOString()}] SSE response received, monitoring connection...`,
          );
          ...
          setTimeout(() => {
            if (!originalClone.body?.locked) {
              console.warn(
                `⚠️ [${new Date().toISOString()}] SSE response body not consumed after 5 seconds`,
              );
            }
          }, 5000);
        }
  ...
  const createSSEReader = useCallback((response: Response) => {
    ...
    console.log(
      `🌊 [${new Date().toISOString()}] Creating SSE reader for response`,
    );
    ...
    return {
      async read() {
        try {
          ...
          if (readerClosed) {
            console.warn(
              `⚠️ [${new Date().toISOString()}] Attempting to read from closed SSE reader`,
            );
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(() => {
              reject(
                new Error(
                  "SSE read timeout - no data received within 120 seconds",
                ),
              );
            }, 120000); // 120 second timeout per read
          });
          ...
          if (done) {
            readerClosed = true;
            console.log(
              `🔚 [${new Date().toISOString()}] SSE stream ended. Total events processed: ${eventCount}`,
            );
            return { done: true, events: [] };
          }
          ...
          for (const line of lines) {
            if (line.startsWith("event: ")) {
              ...
              try {
                ...
                console.log(
                  `📨 [${new Date().toISOString()}] SSE event received:`,
                  {
                    event: currentEvent,
                    ...
                    messageText: data.message?.text
                      ? `"${data.message.text.substring(0, 100)}${data.message.text.length > 100 ? "..." : ""}"`
                      : null,
                ...
                console.error(
                  `❌ [${new Date().toISOString()}] Error parsing SSE data:`,
                  {
                    error:
                      parseError?.message ||
                      parseError?.toString() ||
          ...
          if (
            ...
            console.warn(
              `⚠️ [${new Date().toISOString()}] SSE stream terminated:`,
              errorMessage,
            );
            readerClosed = true;
            return { done: true, events: [] };
          ...
          console.error(
            `❌ [${new Date().toISOString()}] Error reading SSE stream:`,
            {
              error: errorMessage,
          ...
          try {
            ...
            console.warn(
              `⚠️ [${new Date().toISOString()}] Could not release SSE reader lock:`,
              releaseError,
            );
          }
          ...
          (error as any).userMessage =
            "Connection to the agent was lost. Please try again.";
